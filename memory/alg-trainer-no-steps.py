...
def run_problem(agent, p, ptype, ktype, ttype):
    ...
    state, answer = get_state_and_answer(p)
...
def run_agent(parameters):
    ...
    for idx, p in enumerate(study):
        # if idx != 4: continue
        num = 2 if condition == COND_SPSP else 5
        # ptype = PTYPE_DEMO if int(idx / 5) % num == 0 else PTYPE_PRACTICE
        ptype = PTYPE_DEMO if int(idx / 4) % num == 0 else PTYPE_PRACTICE
...
def run_debug(agent, a, b, c):
    ...
    if len(res) == 0:
        ...
        agent.train(state, sai=sai, reward=1)
    else:
        ...
        if correct:
            ...
            agent.train(state, sai=sai, reward=1, rhs_id=res['rhs_id'])
        else:
